...
"""
table_create_log_event = """
    CREATE TABLE IF NOT EXISTS Log_event (
        log_event_date TIMESTAMP NOT NULL,
        log_event_count BIGINT NOT NULL,
        PRIMARY KEY (log_event_date)
    );
"""

create_table_queries = [table_create_repo, table_create_org, table_create_actor, table_create_commit, table_create_payload, table_create_event, table_create_log_event]
...
table_insert_log_event      = "INSERT INTO Log_event VALUES %s ON CONFLICT DO NOTHING;"
...
def _etl(**context):
    ...
    for datafile in all_files:
        ...
        event_cnt.append(event_file_cnt)
...
def _log_event(**context):
    ...
    val = str(log_date), sum(event_cnt)
    sql_insert = table_insert_log_event % str(val)
    cur.execute(sql_insert)
...
with DAG(
    ...
    log_event = PythonOperator(
        task_id = 'log_event',
        python_callable = _log_event,
    )

    get_files >> create_tables >> etl >> log_event